...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for sig in sig_list{
        ...
        let mut report=format!("{} untraced issues(no reply,no label shows its status) in the {} sig\n", sig.number,sig.sig.name);
    ...
    println!("{}",oncall);
    let mut report = format!("{} no-reply issues(no reply,no label shows its status or sig info)\n", un_dispatch_issues);
